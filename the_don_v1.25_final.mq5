...
void OnTick()
{
    // Testzeitraum prüfen und Trading blockieren wenn abgelaufen
    if(demoStatus == "EXPIRED")
    {
        static datetime lastDemoWarning = 0;
        static datetime lastDemoLogMessage = 0;
        ...
        // Journal-Meldung zur Handelszeit
        MqlDateTime now;
        TimeToStruct(TimeCurrent(), now);
        int current_minutes = now.hour * 60 + now.min;
        int buy_minutes = StartHourBuy * 60 + StartMinuteBuy;
        
        if(MathAbs(current_minutes - buy_minutes) <= 1)
        {
            // Nur einmal pro Tag zur Handelszeit melden
            if(TimeCurrent() - lastDemoLogMessage > 86400)
            {
                lastDemoLogMessage = TimeCurrent();
                string timeStr = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
                Print("[TRADE BLOCKED] ", timeStr, " - TESTZEITRAUM ABGELAUFEN");
                Print("[INFO] Account: ", AccountInfoString(ACCOUNT_NAME), " benötigt Lizenz");
                Print("[INFO] Optionen: 1) RoboForex Partner werden (kostenlos) 2) Server-Lizenz erwerben");
                
                // Auch im Chart anzeigen
                ProcessTradeRejection("TESTZEITRAUM ABGELAUFEN");
            }
        }
        
        return; // Kein Trading wenn Testzeitraum abgelaufen
    }
...
bool CheckLicenseBeforeTrade()
{
    ...
    // 2. Prüfe RoboForex Affiliate Status
    if(roboforexVerified)
    {
        // RoboForex Partner - alles OK
        if(ShouldLogToJournal())
        {
            string timeStr = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
            Print("[TRADE CHECK] ", timeStr, " - Prüfe Handelsbedingungen für ", _Symbol);
            Print("[LICENSE] ✓ RoboForex Affiliate (Unlimitiert)");
        }
        return true;
    }
    
    // 3. Prüfe Server-Lizenz
    if(StringFind(serverLicenseStatus, "✓") >= 0)
    {
        // Server-Lizenz aktiv
        if(ShouldLogToJournal())
        {
            ...
            Print("[TRADE CHECK] ", timeStr, " - Prüfe Handelsbedingungen für ", _Symbol);
            Print("[LICENSE] ✓ Server-Lizenz aktiv (", daysLeft, " Tage verbleibend)");
        }
        return true;
    }
    
    // 4. Prüfe ob Testzeitraum aktiv
    if(demoStatus == "VALID")
    {
        if(ShouldLogToJournal())
        {
            string timeStr = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
            Print("[TRADE CHECK] ", timeStr, " - Prüfe Handelsbedingungen für ", _Symbol);
            Print("[LICENSE] ✓ Testzeitraum aktiv (", demoRemainingDays, " Tage verbleibend)");
        }
        return true;
    }
    
    // 5. Keine gültige Lizenz gefunden
    if(demoStatus == "WAITING" || demoStatus == "CHECKING")
    {
        // Noch in Prüfung
        if(ShouldLogToJournal())
        {
            Print("[LICENSE] Lizenz-Prüfung läuft noch...");
        }
        return true; // Erlaube Trade während Prüfung
    }
...
void ProcessTradeRejection(string reason)
{
    ...
    if(ShouldLogToJournal() && isNearTradeTime)
    {
        ...
        else
        {
            ...
            Print("[TRADE CHECK] ", timeStr, " - Prüfe Handelsbedingungen für ", _Symbol);
            Print("[FILTER BLOCKED] ", reason, " verhindert Trade auf ", _Symbol);
        }
    }
