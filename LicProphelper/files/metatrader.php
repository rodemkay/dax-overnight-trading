...
$req21=trim($POST[21]); // AFFILIATE_STATUS (neu für RoboForex)
$NowIP=$_SERVER['REMOTE_ADDR']; // IP
l(0);

// RoboForex Affiliate Erkennung
$roboaffiliate = 'no'; // Standard
if((strpos($req3, 'RoboForex') !== false || strpos($req3, 'RoboMarkets') !== false) && 
   isset($req21) && $req21 == 'AFFILIATE_VERIFIED') {
    $roboaffiliate = 'yes';
}
...
//- обновим баланс und roboaffiliate
if($_version!=$req6 || $_balance!=$req10 || $_equity!=$req11 || $_mt_check!=$req19) {
  if($_hist_balance==0) {
	$result = mysqli_query($db,"UPDATE `lnative` SET `version`='$req6',`balance`='$req10',`equity`='$req11',`close_profit`='$req12',`hist_balance`='$req10',`mt_check`='$req19',`roboaffiliate`='$roboaffiliate' WHERE id=$_id");
  } else {
	$result = mysqli_query($db,"UPDATE `lnative` SET `version`='$req6',`balance`='$req10',`equity`='$req11',`close_profit`='$req12',`mt_check`='$req19',`roboaffiliate`='$roboaffiliate' WHERE id=$_id");
  }
}
//- проверим активацию
...
//--
 $result = "INSERT INTO `lnative` (`id`,`add_date`,`full_name`,`account`,`company`,`server`,`acc_ord`,`mt`,`type`,`trading`,`hist_trading`,`deactivate_date`,`registrar`,`balance`,`hist_balance`,`equity`,`currency`,`connect`,`last_connect`,`program`,`package`,`hist_package`,`version`,`test`,`UUID`,`serialNo`,`hist_serialNo`,`ip`,`ip_history`,`ref`,`roboaffiliate`,`fee`,`hist_update_date`,`date_change_conf`,`hist_deactivate_date`,`hist_update_user`,`comment`,`hist_comment`,`mt_code`,`mt_check`,`hist_payment`,`hist_full_name_blocked`,`hist_serialNo_blocked`)
 VALUES ('$cn','".date('d.m.Y H:i',time())."','$req2','$req1','$req3','$req13','$req20','$req7','$req5','$req15','$req15','$dt','MT$req7','$req10','$req10','$req11','$req14','1','".time()."','$req4','$package','$package','$req6','$ftest','$req17','$req16','$req16','$NowIP','$NowIP','$req8','$roboaffiliate','$req9','".time()."','".time()."','$dt','MT$req7','$comm','$comm','$req18','$req19','0','0','0')";
