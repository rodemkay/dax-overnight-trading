...
<?php
 ...
 if($open = scandir($path)) {
  ...
  foreach($ap as $str) {
    ...
    //--
    if($cList || $cImg) {
	    if(trim(explode("~", $str)[2])!='') {
		 echo '<a href="'.trim(explode("~", $str)[2]).'" target="_blank"><img src="/storage/files/buy.gif" alt="Buy '.trim(explode("~", $str)[0]).'" title="'.trim(explode("~", $str)[0]).'" /></a>';
	    }
	    if(trim(explode("~", $str)[1])!='') {
	     echo '<div style="font-size:24px;font-weight:bold;color:#696cb9;font-family:sans-serif;margin-top:-18px;padding-left:14px"><span>'.trim(explode("~", $str)[1]).'</span> <span style="font-size:18px">USD</span></div>';
	    }
